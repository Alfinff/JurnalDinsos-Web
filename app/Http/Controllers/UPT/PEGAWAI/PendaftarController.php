...
class PendaftarController extends Controller
{
    ...
    public function dataTertunda() {
        ...
        return Datatables:: of($pendaftar)
            ...
            ->addColumn('action', function($dataTertunda){
                $actionBtn = '
                <a class = "btn btn-success" href    = "'.route('upt-pendaftar-tertunda-detail', ['uuid' => $dataTertunda->uuid]).'">Detail</a>
                <a class = "btn btn-primary" onclick = "pilih_pj('.$dataTertunda->id.')" >Hubungi</a>';
    ...
    public function dataTertundaDetailEdit(Request $request, $uuid) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            try {
                ...
                if(isset($request->foto_kondisi)) {
                    $pendaftar['foto_kondisi'] = Str::uuid().".".
                ...
                if(isset($request->surat_pengantar)) {
                    $pendaftar['surat_pengantar'] = Str::uuid().".".
    ...
    public function hubungi(Request $request) {
        ...
        try {
            ...
            $to      = User::where('uuid', $request->idpenanggungjawab)->first();
            $judul   = 'Pendaftar Baru!';
            $pesan = ucwords($to->username) . ', ada pendaftar baru yang perlu di hubungi dari ' . ucwords(auth()->user()->username) . '. Silahkan di Cek!';
            $url    = route('upt-pendaftar-dihubungi');
            $to->notify(new PendaftarNotification($judul, $pesan, $url));
