      ...
      <div class="content">
          <div class="penampungs row">
              ...
              <div class="related col-md-3">
                  ...
                  <div class="cardes">
                  @foreach($semuakegiatan as $nk)
                  <div class="cards">
                      <div class="imgcards">
                        <a href="{{route('halaman-upt-detail-kegiatan', ['uuid' => $detail->upt_id, 'kegiatan_uuid' => $nk->id])}}">
                          @if(!isset($detail->photo))
                              <img title="" src="" alt="">
                          @else
                              <img title="" src="{{Storage::disk('s3')->temporaryUrl($nk->photo, \Carbon\Carbon::now()->addMinutes(3600))}}" alt="">
                          @endif
                        </a>
                      </div>
                      <div class="judulcards">
                        <a href="{{route('halaman-upt-detail-kegiatan', ['uuid' => $detail->upt_id, 'kegiatan_uuid' => $nk->id])}}">
                          <p>{{substr($nk->description,0,50)}}</h2>
                        </a>
                      </div>
                      <div class="time">
